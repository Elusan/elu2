...
# Truth overlay color (semi-transparent orange) — same as base plotting
...
#' @export
my_plot_manage_biomass_panel_sim <- function(rep_man,
                                             scenario_color = NULL,
                                             show_CIs = TRUE,
                                             CI = 0.95,
                                             show_legend = TRUE) {
  ...
  # B/Bmsy-on-B ribbon
  BB_mat <- BB
  t_BB   <- .spict_time_from_par(rep_man, BB_mat)
  df_BB_rib <- if (is.finite(Bmsy_est)) {
    .spict_make_ribbon_df(time = t_BB, lwr = BB_mat[,1] * Bmsy_est, upr = BB_mat[,3] * Bmsy_est)
  } else {
  ...
  # ---- Truth overlays (exactly like plotspict.biomass) ----
  if ("true" %in% names(inp)) {
    # True B(t)
    p <- p + ggplot2::geom_line(
      data = data.frame(time = inp$true$time, y = inp$true$B),
      ggplot2::aes(x = time, y = y),
      inherit.aes = FALSE,
      color = true.col(), linewidth = 0.8
    )
    # True Bmsy: solid orange + black dotted (horizontal line)
    p <- p +
      ggplot2::geom_hline(yintercept = inp$true$Bmsy, color = true.col(), linewidth = 0.8) +
      ggplot2::geom_hline(yintercept = inp$true$Bmsy, color = "black", linetype = "dotted")
  }
