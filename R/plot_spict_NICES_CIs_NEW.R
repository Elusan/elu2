...
#'
#' @inheritParams plot_spict_NICES_CIs_NEW
#' @export
plot_spict_NICES_CIs_NEW <- function(models,
                                     production_fun = NULL,
  ...
  make_biomass_plot <- function(df) {
    ...
    if (!is.null(inp$timeI) && length(inp$timeI) >= 1) {
      ...
      p <- p + geom_point(data = obs1, aes(x = time, y = obs),
                          color = dot_blue, shape = 16, size = 2.5, inherit.aes = FALSE)
    }
    if (!is.null(inp$timeI) && length(inp$timeI) >= 2) {
      ...
      p <- p + geom_point(data = obs2, aes(x = time, y = obs),
                          ...
                          size = 2.5, stroke = 0.5, inherit.aes = FALSE)
    }
    p
  }
