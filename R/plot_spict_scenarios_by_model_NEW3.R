plot_spict_scenarios_by_model_NEW3 <- function(models,
  ...
  make_plot <- function(df, ylab_expr, hline = NULL) {
    ...
    if (show_CIs) {
      p <- p + geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.22, color = NA, show.legend = FALSE)
    }
  ...
  if (!is.null(extract_catch_data)) {
    ...
    plots$catch <- ggplot() +
      { if (show_CIs) geom_ribbon(data = predicted, aes(x = time, ymin = lwr, ymax = upr, fill = model),
                                  alpha = 0.22, show.legend = FALSE) } +
      geom_line(data = predicted, aes(x = time, y = catch, color = model), linewidth = lindwd) +
      geom_point(data = observed, aes(x = time, y = catch, color = model),
