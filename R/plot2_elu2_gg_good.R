...
plot2_elu2_gg_good <- function(rep_man,
  ...
  sc_core   <- intersect(sc_order, sc_present)      # keeps sc_order order
  sc_other  <- setdiff(sc_present, sc_core)
  scs_final <- c(sc_core, sort(sc_other))

  # Back-compat so older code that uses `scs` keeps working
  scs <- scs_final
...
kobe_all_in_one_gg <- function(rep,
  ...
  p <- p + ggplot2::theme_bw() +
    ggplot2::theme(
      panel.grid = ggplot2::element_blank(),
      panel.border = ggplot2::element_rect(color="gray45", fill=NA, linewidth=0.8),
