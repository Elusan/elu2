...
#'
#' @title Multi-panel SPiCT management plot with Kobe panel (ggplot2)
#'
#' @description
...
#' - Historical base (≤ last observation) **B/Bmsy** and **F/Fmsy** appear in
#'   SPiCT-blue with CIs; scenario lines are overlaid in user-supplied or
#'   default colors.
...
#'   to relative.
#'
#' @param rep_man A fitted `spictcls` object for which [spict::manage()] has been run.
...
#'   `names(rep_man$man)`. Unnamed vectors are matched by order.
...
#'
#' @return A patchwork object (default) or a named list of ggplot objects.
#'
#' @details
#' **Conventions**
#' - Observed catches are plotted at `timeC` (start of interval);
#'   predicted catches at `timeCpred` (SPiCT convention).
#' - Vertical lines in time-series panels are solid grey (thin).
#' - In the Catch panel, the left line is the **last observed** catch start
#'   (tail of `timeC`), and the right line is the **management start**.
#'
#' **Saving**
#' - A PNG is automatically written to `FIG/Manage/manage_best_model.png`
#'   with size 14×8 inches at 300 dpi.
#'
#' @examples
#' \dontrun{
#'   library(spict)
...
#' @export
plot2_elu2_gg_good_AJUSTED.EE <- function(rep_man,
                                          scenario_color = NULL,
                                          show_CIs = TRUE,
                                          CI = 0.95,
  ...
  # ---- Canonical SPiCT scenario order + labels ----
  ...
  # What scenarios exist in this run?
  sc_present <- names(rep_man$man)
  if (is.null(sc_present)) sc_present <- character(0)

  # Order = SPiCT canonical first, then any others (sorted for stability)
  ...
  # Make scenario a factor everywhere so legends follow this order
  for (nm in c("bbmsy","ffmsy","catch_pred")) {
    if (!is.null(dat[[nm]]) && nrow(dat[[nm]]) > 0) {
      dat[[nm]]$scenario <- factor(dat[[nm]]$scenario, levels = scs_final)
    }
  }

  # Colors aligned to this ordered set
  if (is.null(scenario_color)) {
    cols <- man_cols(length(scs_final)); names(cols) <- scs_final
  } else {
    cols <- scenario_color
  ...
  # Legend labels aligned to the same order (fallback = raw name)
  ...
  pB <- ggplot2::ggplot() +
    ...
    add_management_vlines_BF_good(rep_man, color = "grey75", linetype = "solid", linewidth = 0.4, lineend = "butt") +
  ...
  pF <- ggplot2::ggplot() +
    ...
    # Draw management vlines under the scenario paths so lines stay visible
    add_management_vlines_BF_good(rep_man, color = "grey75", linetype = "solid", linewidth = 0.4, lineend = "butt") +
    ggplot2::geom_line(
      data = dat$ffmsy,
      ggplot2::aes(x = time, y = est, color = scenario),
  ...
  pC <- ggplot2::ggplot() +
    ...
    add_management_vlines_catch_good(rep_man, color = "grey75", linetype = "solid", linewidth = 0.4, lineend = "butt") +
  ...
  # ---- Compose the 2x2 grid (always compose so we can save it) ----
...
# ---- NEW, uniquely-named helpers to avoid masking ----
...
#' Left = last observed catch start (tail of timeC); Right = management start.
#' @keywords internal
#' @noRd
...
#' @keywords internal
#' @noRd
kobe_all_in_one_gg.EE <- function(rep,
                                  logax = FALSE,
                                  plot.legend = TRUE,
                                  man.legend = TRUE,
                                  ext = TRUE,
  ...
  # ensure these exist even when `rep$man` is missing/empty
  df_man      <- data.frame(x = numeric(0), y = numeric(0), scenario = character(0))
  df_man_int  <- data.frame(x = numeric(0), y = numeric(0), scenario = character(0))
  df_man_jump <- data.frame(x = numeric(0), xend = numeric(0), y = numeric(0), yend = numeric(0),
  ...
  # Scenarios (management path, intermediate pre-management path, and oblique jump)
  ...
  if ("man" %in% names(rep) && length(rep$man)) {
    ...
    for (i in seq_len(nman)) {
      ...
      if (min(inp$dtc) < 1) {
        ...
        keep_man <- (ttA >= t0) & (ttA <= tE)
        keep_int <- (ttA >= lastobs) & (ttA < t0)
        pre_mask <- ttA < t0
      } else {
        bx <- estB/bscal; fy <- estF/fscal; ttA <- tt
        keep_man <- (tt >= t0) & (tt <= tE)
        keep_int <- (tt >= lastobs) & (tt < t0)
        pre_mask <- tt < t0
      }

      # Management and pre-management paths
      x_man <- bx[keep_man]; y_man <- fy[keep_man]
      x_int <- bx[keep_int]; y_int <- fy[keep_int]

      n_man <- length(x_man); n_int <- length(x_int)

      # Determine exact endpoints used for the jump
      ...
      x1 <- if (n_man > 0) x_man[1L] else NA_real_
      y1 <- if (n_man > 0) y_man[1L] else NA_real_

      # Ensure the dashed pre path ends exactly at (x0,y0)
      if (is.finite(x0) && is.finite(y0)) {
        if (n_int == 0L || ! (same_pt(utils::tail(x_int,1), x0) && same_pt(utils::tail(y_int,1), y0))) {
          x_int <- c(x_int, x0); y_int <- c(y_int, y0); n_int <- length(x_int)
        }
      }

      # Build data frames
      lst_path[[i]]  <- data.frame(x = x_man,  y = y_man,  scenario = if (n_man) rep(leg_man[i], n_man) else character(0))
      lst_ipath[[i]] <- data.frame(x = x_int, y = y_int, scenario = if (n_int) rep(leg_man[i], n_int) else character(0))

      # Oblique jump (no gap): connect (x0,y0) -> (x1,y1)
      if (is.finite(x0) && is.finite(y0) && is.finite(x1) && is.finite(y1) &&
          (abs(x1 - x0) > 0 || abs(y1 - y0) > 0)) {
        lst_jump[[i]] <- data.frame(x = x0, xend = x1, y = y0, yend = y1, scenario = leg_man[i])
      } else {
  ...
  # ---- historic base path (solid blue), rectangular ends/joins ----
  if (nrow(df_traj) > 1) {
    p <- p + ggplot2::geom_path(
      data = df_traj, ggplot2::aes(x = x, y = y),
      color = grDevices::rgb(0, 0, 1, 0.8),
      linewidth = 0.5, lineend = "butt", linejoin = "mitre"
    )
  } else if (nrow(df_traj) == 1) {
    p <- p + ggplot2::geom_point(
      data = df_traj, ggplot2::aes(x = x, y = y),
      color = grDevices::rgb(0, 0, 1, 0.8), size = 1.5
    )
  }

  # ---- optional dashed predicted path + EB∞ segment (rectangular) ----
  if (!is.null(df_pred)) {
    if (nrow(df_pred) > 1) {
      p <- p + ggplot2::geom_path(
        data = df_pred, ggplot2::aes(x = x, y = y),
        linetype = "33", color = grDevices::adjustcolor("blue", 0.8),
        linewidth = 0.9, lineend = "butt", linejoin = "mitre"
      )
    } else if (nrow(df_pred) == 1) {
      p <- p + ggplot2::geom_point(
        data = df_pred, ggplot2::aes(x = x, y = y),
        color = grDevices::adjustcolor("blue", 0.8), size = 1.5
      )
    }
    p <- p + ggplot2::geom_segment(
      data = df_EBseg, ggplot2::aes(x = x, xend = xend, y = y, yend = yend),
      color = "blue", linetype = "33",
  ...
  # ---- pre-management dashed path (draw FIRST) ----
  if (!is.null(df_man_int)  && nrow(df_man_int)) {
    ...
    if (any(dfmi$..n.. > 1)) {
      p <- p + ggplot2::geom_path(
        ...
        linetype = "33", linewidth = 0.9,
        lineend = "butt", linejoin = "mitre",
        show.legend = man.legend
      )
    }
    if (any(dfmi$..n.. == 1)) {
      p <- p + ggplot2::geom_point(
        data = dfmi[dfmi$..n.. == 1, , drop = FALSE],
        ggplot2::aes(x = x, y = y, color = scenario),
        size = 1.5, show.legend = man.legend
      )
    }
  }

  # ---- solid management path (draw SECOND) ----
  ...
  # ---- vertical/oblique jump (draw LAST to close any dash gap, no overhang) ----
  if (!is.null(df_man_jump) && nrow(df_man_jump))
    p <- p + ggplot2::geom_segment(
      data = df_man_jump,
      ggplot2::aes(x = x, xend = xend, y = y, yend = yend, color = scenario),
      linewidth = 0.9, lineend = "butt",
      show.legend = man.legend
    )

  p <- p +
    ggplot2::geom_point(data = df_first, ggplot2::aes(x=x,y=y), shape=21, fill="white", color="black", size=3.3, stroke=0.5) +
    ggplot2::geom_text (data = df_first, ggplot2::aes(x=x,y=y,label=lab), vjust=-0.8, size=3) +
    ggplot2::geom_point(data = df_last,  ggplot2::aes(x=x,y=y), shape=22, fill="white", color="black", size=3.3, stroke=0.5) +
  ...
  sec_factor_B  <- (utils::tail(Bmsy,1)[2] / if (rel.axes) Bmsy[2] else 1)
  if (isTRUE(logax)) {
    p <- p + ggplot2::scale_x_log10(
      limits = xlim,
      name   = if (is.null(xlabel)) if (rel.axes) expression(B[t]/B[MSY]) else add_catchunit_loc(expression(B[t]), inp$catchunit) else xlabel,
      labels = function(x) formatC(x, format="f", digits=0),
      expand = c(0,0),
      sec.axis = if (!rel.axes && isTRUE(ext)) ggplot2::sec_axis(~ . / sec_factor_B, name = expression(B[t]/B[MSY]), labels = lab_rel_fmt) else ggplot2::waiver()
    )
    p <- p + ggplot2::scale_y_log10(
      limits = ylim, name = if (rel.axes) expression(F[t]/F[MSY]) else expression(F[t]), labels = function(x) formatC(x, format="f", digits=1), expand = c(0,0),
      sec.axis = if (!rel.axes && isTRUE(ext)) ggplot2::sec_axis(~ . / utils::tail(Fmsy,1)[2], name = expression(F[t]/F[MSY]), labels = lab_rel_fmt) else ggplot2::waiver()
    )
  } else {
  ...
  p <- p + ggplot2::theme_bw() +
    ggplot2::theme(
      panel.grid = ggplot2::element_blank(),
      panel.border = ggplot2::element_rect(color="gray45", fill=NA, linewidth=1.2),
...
add_management_vlines_BF <- function(rep,
                                     ...
                                     linewidth = 0.6,
                                     lineend = "butt") {
  # Forward to the new helper that supports `lineend`
  add_management_vlines_BF_good(rep,
                                color = color,
                                linetype = linetype,
                                linewidth = linewidth,
                                lineend = lineend)
...
utils::globalVariables(c(
  ...
  "index","obsrel","x","y","xend","yend","xmin","xmax","ymin","ymax","lab","..n.."
))
