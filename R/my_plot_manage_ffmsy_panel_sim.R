#' Repeatable colour palette for management scenarios
#'
#' @description
...
get_base_BB_FF_pre <- function(rep_man, CI = 0.95) {
  ...
  BB <- try(elu2::get.par("logBBmsy", rep_man, exp = TRUE), silent = TRUE)
  FF <- try(elu2::get.par("logFFmsy", rep_man, exp = TRUE), silent = TRUE)
...
prepare_manage_panel_data <- function(rep_man, CI = 0.95) {
  ...
  if (has_man) {
    ...
    for (i in seq_len(ns)) {
      ...
      BB <- try(elu2::get.par("logBBmsy", rp, exp = TRUE), silent = TRUE)
      FF <- try(elu2::get.par("logFFmsy", rp, exp = TRUE), silent = TRUE)
      ...
      CP  <- try(elu2::get.par("logCpred", rp, exp = TRUE), silent = TRUE)
...
#' If `rep_man$inp$true` exists (simulation), overlays the true `Fs/Fmsy`
#' trajectory in orange, without altering CIs, limits, or legends.
#'
...
#' @export
my_plot_manage_ffmsy_panel_sim <- function(rep_man,
                                           scenario_color = NULL,
                                           show_CIs = TRUE,
                                           CI = 0.95,
                                           show_legend = FALSE) {
  ...
  # --- Optional simulation truth overlay: Fs/Fmsy in orange
  df_true <- NULL
  if (!is.null(rep_man$inp$true)) {
    tr <- rep_man$inp$true
    if (!is.null(tr$time) && !is.null(tr$Fs) && !is.null(tr$Fmsy)) {
      vals <- tr$Fs / tr$Fmsy
      ok   <- is.finite(tr$time) & is.finite(vals)
      if (any(ok)) {
        df_true <- data.frame(time = tr$time[ok], val = vals[ok])
      }
    }
  }
  true_col <- grDevices::rgb(1, 165/255, 0)  # orange, like base plotting's true.col()
  ...
  pF <- ggplot2::ggplot() +
    ...
    # --- draw simulation truth (Fs/Fmsy) if available ---
    { if (!is.null(df_true) && nrow(df_true)) ggplot2::geom_line(
      data = df_true,
      ggplot2::aes(x = time, y = val),
      inherit.aes = FALSE, color = true_col, linewidth = 0.9
    ) } +
  ...
  if (!has_man) {
    FF_all <- try(elu2::get.par("logFFmsy", rep_man, exp = TRUE, CI = CI), silent = TRUE)
    if (!inherits(FF_all, "try-error")) {
      ...
      if (nrow(df_pr)) {
        ...
        # Bridge from last in-sample to first predicted
