...
#'
#' Produces a Kobe-style phase plot either in absolute units (\eqn{B_t} vs \eqn{F_t})
#' with secondary relative axes, or directly in relative units (\eqn{B_t/B_{MSY}} vs
#' \eqn{F_t/F_{MSY}}). Fills quadrants, draws the MSY ellipse, historical trajectory,
#' optional prediction segment to \eqn{E(B_\infty)}, scenario trajectories (if
#' \code{rep$man} present), and a compact \eqn{\\bold{E(B_{\\infty})}} mini-legend.
#' True trajectories are not drawn; the true reference point is shown if available.
#'
#' @param model Fitted SPiCT object (\code{spictcls}) with \code{inp$reportmode == 0}.
#' @param logax Logical; log10 scales for both axes.
#' @param plot.legend Logical; show the small \eqn{E(B_\infty)} legend.
#' @param man.legend Logical; show a legend for management scenarios.
#' @param ext Logical; add secondary relative axes when \code{rel.axes = FALSE}.
#' @param rel.axes Logical; force plotting in relative units (disables \code{ext}).
#' @param xlim,ylim Optional numeric length-2 axis limits.
#' @param labpos Reserved integer vector for first/last year label offsets.
#' @param xlabel Optional x-axis label override.
#' @param stamp Optional stamp (e.g., version tag) at lower-right.
#' @param verbose Logical; emit extraction warnings.
#' @param CI Confidence level for intervals (0–1).
#' @param print_it Logical; print the plot as a side effect.
#'
...
#' plot_elu2_panel_kobe(m1, rel.axes = TRUE)
#' }
plot_elu2_panel_kobe <- function(model,
  ...
  make_rp_ellipse <- function(rep) {
    ...
    if (requireNamespace("ellipse", quietly = TRUE)) {
      ellipse::ellipse(stats::cov2cor(covBF)[1, 2], scale = sqrt(diag(covBF)), centre = parBF, npoints = 300)
  ...
  get_EBinf <- function(rep) {
    ...
    if (min(rep$inp$dtc) < 1) { alf <- annual_avg(rep$inp$time, logFs[, 2]); fff <- exp(alf$annvec) }
  ...
  bx <- if (rel.axes) 1 else Bmsy[2] / (if (rel.axes) Bmsy[2] else 1); bx <- clamp(bx, xminq, xmaxq)
  fy <- if (rel.axes) 1 else Fmsy[2] / (if (rel.axes) Fmsy[2] else 1); fy <- clamp(fy, yminq, ymaxq)
  ...
  # --------------------------- Build ggplot ------------------------------ #
  p <- ggplot2::ggplot()
  p <- p + ggplot2::geom_rect(data = df_green_rect,   ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                              fill = grDevices::adjustcolor("darkseagreen3", alpha.f = 1), color = NA)
  p <- p + ggplot2::geom_rect(data = df_yellowL_rect, ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                              fill = grDevices::adjustcolor("khaki1", alpha.f = 1), color = NA)
  p <- p + ggplot2::geom_rect(data = df_yellowT_rect, ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                              fill = grDevices::adjustcolor("khaki1", alpha.f = 1), color = NA)
  p <- p + ggplot2::geom_rect(data = df_red_rect,     ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
  ...
  if (nrow(df_ellipse) > 1) {
    ...
    p <- p + ggplot2::geom_point(data = df_ellipse, ggplot2::aes(x = x, y = y), color = "gray40", size = 2)
  }
  ...
  if (!is.null(df_EBseg)) {
    p <- p + ggplot2::geom_segment(data = df_EBseg,
      ...
      ggplot2::geom_point(data = data.frame(x = df_EBseg$xend, y = df_EBseg$yend),
                          ggplot2::aes(x = x, y = y), shape = 23, fill = "gold", color = "black", size = 2, stroke = 0.5)
  }
  ...
  if (!is.null(df_msy_prev) && nrow(df_msy_prev)) p <- p + ggplot2::geom_point(data = df_msy_prev, ggplot2::aes(x = x, y = y), shape = 3, color = "magenta", size = 2)
  if (!is.null(df_msy_curr) && nrow(df_msy_curr)) p <- p + ggplot2::geom_point(data = df_msy_curr, ggplot2::aes(x = x, y = y), shape = 3, color = "black", size = 2)
  ...
  if (isTRUE(logax)) {
    p <- p + ggplot2::scale_x_log10(
      ...
      ggplot2::scale_y_log10(
        limits = ylim, name = ylab_final, labels = fmt1, expand = c(0, 0),
        sec.axis = if (!rel.axes && isTRUE(ext)) ggplot2::sec_axis(
          trans  = ~ . / (Fmsy[2] / 1),
  ...
  if (!is.null(df_man) && nrow(df_man) && man.legend) {
    ...
    colv <- man_cols()[seq_along(uniq_sc)]; names(colv) <- uniq_sc
  ...
  if (plot.legend && !(min(rep$inp$dtc) < 1)) {
    ...
    p <- p + ggplot2::annotate("point", x = px - gap, y = py, shape = 23, size = 2.5, fill = "gold", color = "black", stroke = 0.6) +
      ggplot2::annotate("text", x = px, y = py, label = "bold(E(B[infinity]))", parse = TRUE, hjust = 1, vjust = 0.5, size = 4)
  }

  # Non-convergence badge
  if (rep$opt$convergence != 0) {
    p <- p + ggplot2::annotate("text", x = xlim[1] + 0.01 * diff(xlim), y = ylim[2] + 0.02 * diff(ylim),
                               label = "▲", color = "black", size = 6) +
      ggplot2::annotate("text", x = xlim[1] + 0.01 * diff(xlim), y = ylim[2] + 0.02 * diff(ylim),
                        label = "!", color = "black", size = 3, vjust = 0.35)
  }

  # Optional stamp
  if (!is.null(stamp) && nzchar(stamp)) {
    p <- p + ggplot2::annotate("text", x = xlim[2] - 0.01 * diff(xlim), y = ylim[1] - 0.04 * diff(ylim),
