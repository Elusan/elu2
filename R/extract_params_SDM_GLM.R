...
#' `make_spict_summary_df1_nopipe()` to create per-scenario summary tables
#' and then reshapes and filters to the requested parameters.
#'
#' @section Required input format:
#' - **Named** `model_list` with names that match `"<scenario><family>.<type>"`; e.g.:
#'   `"S1F.SDM"`, `"S1S.GLM"`, `"S1P.SDM"`, … where
#'   `scenario = S\\d+`, `family ∈ {F,S,P}` (Fox, Schaefer, Pella),
#'   and `type ∈ {SDM, GLM}`.
#' - `make_spict_summary_df1_nopipe()` must be available in the search path
...
#' `make_spict_summary_df1_nopipe()` and maps them to the keys
#' `"K"`, `"r"`, `"MSY"`, `"B/Bmsy"`, `"F/Fmsy"`.
#' If a ratio label embeds a year (e.g., `$B_{2022}/B_{MSY}$`), the year
#' is extracted into the `year` column; otherwise `year` is left
#' as `NA_integer_`. Parameters not in the requested set are discarded.
#'
#' @param model_list A **named** flat list of SPiCT fits whose names follow
#'   the pattern `<scenario><family>.<type>` (e.g., `"S3F.SDM"`).
#' @param scenarios Optional character vector of scenarios to include
#'   (e.g., `c("S1","S2")`). If `NULL`, scenarios are auto-detected
#'   from `names(model_list)`.
#' @param verbose Logical; print progress messages per scenario. Default `TRUE`.
#'
#' @return
#' A tidy data frame with columns:
#' - `scenario` — Scenario ID (e.g., `"S1"`).
#' - `type` — Model type (`"SDM"` or `"GLM"`).
#' - `family` — `"Fox"`, `"Schaefer"`, or `"Pella"`.
#' - `param` — one of `"K"`, `"r"`, `"MSY"`, `"B/Bmsy"`, `"F/Fmsy"`.
#' - `year` — Year embedded in a ratio label (if any), otherwise `NA_integer_`.
#' - `est` — Point estimate (numeric).
#' - `lo` — Lower 95\\% CI (numeric).
#' - `hi` — Upper 95\\% CI (numeric).
#'
#' Factor levels are set to `family = c("Fox","Schaefer","Pella")`,
#' `type = c("SDM","GLM")`, and
...
#' }
#'
#' @seealso [make_spict_summary_df1_nopipe()]
#' @export
