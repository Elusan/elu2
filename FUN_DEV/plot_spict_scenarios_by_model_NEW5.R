plot_spict_scenarios_by_model_NEW5 <- function(models,
  ...
  # Helper to get time series from models (same as NEW3)
  ...
  # Biomass panel with observed indices (as in NEW3)
  make_biomass_plot <- function(df) {
    ...
    first_model <- models[[1]]  # assume shared inp structure
  ...
  # NEW: B/Bmsy panel with observed indices transformed to relative scale
  make_bbmsy_plot <- function(df) {
    ...
    # Add observed indices like SPiCT, but in relative units: (obsI/q) / Bmsy
    first_model <- models[[1]]
    qest  <- get.par("logq", first_model, exp = TRUE)
    Bmsy  <- get.par("logBmsy", first_model, exp = TRUE)  # vector (ll, est, ul, ...)
    Bmsy2 <- if (is.null(nrow(Bmsy))) Bmsy[2] else Bmsy[1, 2]  # point estimate

    inp <- first_model$inp

    if (length(inp$timeI) >= 1) {
      obs1_rel <- data.frame(time = inp$timeI[[1]],
                             obs  = (inp$obsI[[1]] / qest[inp$mapq[1], 2]) / Bmsy2)
      p <- p + geom_point(data = obs1_rel, aes(x = time, y = obs),
                          color = "blue", shape = 16, size = 2, inherit.aes = FALSE)
    }
    if (length(inp$timeI) >= 2) {
      obs2_rel <- data.frame(time = inp$timeI[[2]],
                             obs  = (inp$obsI[[2]] / qest[inp$mapq[2], 2]) / Bmsy2)
  ...
  # Generic plot maker for other panels (unchanged from NEW3)
