#' Per–data type retrospective grid (G4) with identical ggplot settings
#' to plot_retro_grid.G4_B_et_F2_Corrected_one_CI()
#'
#' @param models Named list of fitted SPiCT models for a single scenario (each with $retro)
#' @param scenario_name Character title (e.g., "Scenario 1")
#' @param data_type One of c("SDM","GLM"); matched against model names ending in ".SDM"/"_SDM" or ".GLM"/"_GLM"
#' @param peel_colors,palette,width,height,dpi,output_dir,filename Same params as the base function
#' @return A patchwork ggplot object; optionally writes a PNG if output_dir is provided
#' @export
plot_retro_grid.G4_B_et_F2_Corrected_one_CI_by_dtype <- function(
    models,
    scenario_name = "Scenario 1",
    data_type = c("SDM", "GLM"),
    peel_colors = NULL,
    palette = "JABBA",
    width = 20, height = 12, dpi = 400,
    output_dir = NULL, filename = NULL
) {
  data_type <- match.arg(data_type)

  # ---- Filter models by data type suffix (.SDM/_SDM or .GLM/_GLM) -----------
  if (is.null(models) || !length(models) || is.null(names(models))) {
    stop("`models` must be a non-empty *named* list for one scenario.", call. = FALSE)
  }
  pat <- paste0("([._])", data_type, "$")
  keep <- grepl(pat, names(models))
  models <- models[keep]
  if (!length(models)) stop("No models matched data type '", data_type, "'.", call. = FALSE)
  ...
  # ---- EXACT same theme block as base function ------------------------------
  .apply_theme <- local({
      ...
      function(base_size = 8, base_family = "") {
        ggplot2::theme_minimal(base_size = base_size, base_family = "") +
          ggplot2::theme(
            ...
            panel.border     = ggplot2::element_rect(fill = NA, colour = "grey45", linewidth = 0.8),
  ...
  # ---- EXACT same panel builder & data wrangling as base --------------------
  make_model_panels <- function(model_fit, model_nm, peel_colors_input) {
    ...
    make_panel <- function(df, ylab, rho_text = NULL) {
      ...
      p <- ggplot2::ggplot() +
        { if (nrow(df_all)) ggplot2::geom_ribbon(
          data = df_all,
          ggplot2::aes(x = .data$time, ymin = .data$lower, ymax = .data$upper),
          fill = ci_gray, alpha = 0.25, inherit.aes = FALSE
        ) else NULL } +
        ggplot2::geom_line(
          data = df,
          ggplot2::aes(x = .data$time, y = .data$estimate, color = .data$peel),
          linewidth = 0.6
        ) +
        ...
        { if (!is.null(rho_text)) ggplot2::annotate(
  ...
  # ---- Legend subtitle (same: expects make_peel_subtitle) -------------------
  if (!.exists_fun("make_peel_subtitle")) {
    stop("`make_peel_subtitle()` must exist to build the legend.", call. = FALSE)
  }
  subtitle_html <- get("make_peel_subtitle", inherits = TRUE)(subtitle_peel_names, subtitle_peel_cols)

  # ---- Final assembly with IDENTICAL spacing & text settings -----------------
  scen_title <- paste0(scenario_name, " — ", data_type)

  final <- grid_plot +
    patchwork::plot_layout(guides = "collect") &
    ggplot2::theme(
      plot.background = ggplot2::element_rect(fill = "white", color = NA),
      # vertical space below title (Title ↔ Legend)
      ...
      # vertical space above & below legend (Legend ↔ Plots)
  ...
  # ---- Optional save (same logic, with dtype suffix if filename provided) ----
  if (!is.null(output_dir)) {
    if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
    filename2 <- filename
    if (!is.null(filename2) && nzchar(filename2)) {
      dot <- regexpr("\\.[^\\.]*$", filename2)
      if (dot > 0) {
        stem <- substr(filename2, 1, dot - 1)
        ext  <- substr(filename2, dot, nchar(filename2))
        filename2 <- paste0(stem, "_", data_type, ext)
      } else filename2 <- paste0(filename2, "_", data_type, ".png")
    } else {
      scen_lab <- gsub("[^A-Za-z0-9]+", "_", scen_title)
      filename2 <- paste0("retrospective_", scen_lab, ".png")
    }
    outpath <- file.path(output_dir, filename2)
