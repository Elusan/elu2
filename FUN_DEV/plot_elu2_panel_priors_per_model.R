...
#'   single panel immediately (convenient for embedding in multi-panel layouts).
#'   Default `NULL` (all active priors).
#' @param stamp Optional message (unused in plot; emitted when returning a grid).
...
#' \dontrun{
#' rep <- fit.spict(inp)
#' p1  <- plot_elu2_panel_priors.elu6_per_model(rep, do.plot = 1)  # first prior
#' pA  <- plot_elu2_panel_priors.elu6_per_model(rep)               # grid of priors
#' }
#'
#' @import ggplot2
...
#' @export
plot_elu2_panel_priors_per_model <- function(rep, model_id = NULL,
                                                  do.plot = NULL,
                                                  stamp = get.version(),
                                                  CI = 0.95) {
  inp <- rep$inp
  ...
  #' Minimal compact theme for density panels (internal)
  #' @keywords internal
  #' @noRd
  ...
  #' Simple formatter for optional index suffix (internal)
  #' @keywords internal
  #' @noRd
  fd <- function(x) {
  ...
  #' Add catch unit to a plain-text label (internal; ggplot-friendly)
  #' @keywords internal
  #' @noRd
  ...
  if (ninds > 0) {
    for (i in seq_len(ninds)) {
      ...
      for (rr in seq_len(nrow(par))) {
        ...
        # Enforce do.plot cap (single-panel early return)
