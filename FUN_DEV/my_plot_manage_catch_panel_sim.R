...
#'
#' When `rep_man$inp$true` is present (simulations), a **horizontal “True MSY”**
#' overlay is drawn exactly like `plotspict.catch()`: first a solid
#' semi-transparent orange line (`true.col()`), then a black dotted line on top.
#' No legend entry is added for this overlay.
#'
...
#' @export
my_plot_manage_catch_panel_sim <- function(rep_man,
                                           scenario_color = NULL,
                                           show_CIs = TRUE,
                                           CI = 0.95,
                                           show_legend = FALSE) {
  ...
  if (is.null(msy_df)) {
    MSYone <- try(get.par("logMSY", rep_man, exp = TRUE, CI = CI), silent = TRUE) # c(ll, est, ul)
  ...
  pC <- ggplot2::ggplot() +
    ...
    # ---------- True MSY overlay (matches plotspict.catch behavior) ----------
  {
    if ("true" %in% names(rep_man$inp) && !is.null(rep_man$inp$true) &&
        !is.null(rep_man$inp$true$MSY) && is.finite(rep_man$inp$true$MSY)) {
      list(
        ggplot2::geom_hline(yintercept = rep_man$inp$true$MSY,
                            color = true.col(), linewidth = 0.8, show.legend = FALSE),
        ggplot2::geom_hline(yintercept = rep_man$inp$true$MSY,
