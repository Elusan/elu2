...
#' @export
my_plot_kobe_all_management_scenario_sim <- function(rep,
                                                     logax = FALSE,
                                                     plot.legend = TRUE,
                                                     man.legend = TRUE,
                                                     ext = TRUE,
  ...
  if ("man" %in% names(rep) && length(rep$man)) {
    ...
    while (i <= nman) {
      rp <- rep$man[[i]]
      estF <- get_par("logFs", rp, exp = TRUE)[, 2]   # <-- (logFs) consistent with main
  ...
  # ----------------- Compact top-right mini-legend (LEFT-aligned labels) -----------------
  # Matches your reference: bold E(B∞) and "True" (if available), tight spacing.
  if (plot.legend) {
    dx <- diff(xlim); dy <- diff(ylim)
    px_right <- xlim[2] - 0.015 * dx     # a touch in from the right
    gapx     <- 0.10 * dx                # marker offset from right
    txtpad   <- 0.02 * dx                # text offset from marker
    vgap     <- 0.06 * dy                # row spacing

    x_marker <- px_right - gapx
    x_text   <- x_marker + txtpad        # LEFT-aligned text
    ...
    if (!is.null(df_true)) {
      ...
      p <- p +
        ggplot2::annotate("point",
                          x = x_marker, y = y_true,
                          shape = 25, size = 2.4,
                          fill  = grDevices::rgb(1, 165/255, 0, alpha = 0.7),
