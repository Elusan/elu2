...
#' `check.rep()`, `get.par()`, and `get.version()`. A compact theme helper
#' is inlined (internal) and applied at the end.
#'
#' @examples
#' \dontrun{
#' rep <- fit.spict(inp)
...
#'
#' @import ggplot2
#' @importFrom spict check.rep get.par get.version
...
plot_elu2_panel_production <- function(rep, n.plotyears = 40,
  ...
  #' Compact minimal theme for ELU2 panels (internal)
  #' @keywords internal
  #' @noRd
  ...
  p <- ggplot2::ggplot() +
    ...
    # Production trajectory (time-ordered)
    {
      if (!is.null(df_path)) {
        list(
  ...
  # Compact theme (kept) + in-panel legend styling (no legend unless mapped)
  p <- p +
    ...
    ggplot2::theme(
      ...
      # restore black border & centered title
      panel.border         = ggplot2::element_rect(color = "black", fill = NA, linewidth = 1),
      plot.title           = ggplot2::element_text(hjust = 0.5)
    )
