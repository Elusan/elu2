...
# --- Truth overlay color (semi-transparent orange), exactly as in plotspict.*
#' @keywords internal
#' @noRd
true_col <- function() grDevices::rgb(1, 165/255, 0, alpha = 0.7)
...
#' @return A data frame with columns `index`, `time`, and `obsrel`, or `NULL` if no indices are present.
#' @details
#' Uses `spict::get.par("logq", ...)` and `spict::get.par("logBmsy", ...)` to scale raw
#' observations in `rep_main$inp$obsI`. Supports up to two indices.
#' @keywords internal
#' @noRd
obs_indices_rel_df <- function(rep_main) {
  stopifnot(inherits(rep_main, "spictcls"))
  # get q and Bmsy (always use "est" = column 2)
  ...
  # Take the last-row estimate if multiple; otherwise the scalar estimate
  ...
  # Helper to compute obsrel safely
  ...
  if (!is.null(inp$timeI) && length(inp$timeI) >= 1L &&
    ...
    k <- k + 1L; out_list[[k]] <- .one_index(1L)
  }
  if (!is.null(inp$timeI) && length(inp$timeI) >= 2L &&
      !is.null(inp$obsI)  && length(inp$obsI)  >= 2L) {
    k <- k + 1L; out_list[[k]] <- .one_index(2L)
  }

  out_list <- Filter(NROW, out_list)
  if (!length(out_list)) NULL else do.call(rbind, out_list)
}
...
#' @return List with `t_last_obs`, `BB`, `FF` data frames.
#' @keywords internal
#' @noRd
get_base_BB_FF_pre <- function(rep_man, CI = 0.95) {
  ...
  if (!length(tt)) return(list(t_last_obs = NA_real_,
                               BB = data.frame(), FF = data.frame()))
...
#'
#' @description
#' Creates tidy data frames for plotting B/Bmsy and (optionally) other panels.
#' If `rep_man$man` is not present, returns empty scenario frames while still
#' providing observed catch and observed indices (relative scale).
#'
#' @param rep_man A fitted SPiCT report object (`spictcls`), possibly containing `man`.
...
prepare_manage_panel_data <- function(rep_man, CI = 0.95) {
  ...
  # Observed catch series (if present)
  ...
  if (has_man) {
    ...
    for (i in seq_len(ns)) {
      ...
      # Prefer rownames if numeric; otherwise fall back to rp$inp$time
      ...
      CP <- spict::get.par("logCpred", rp, exp = TRUE)
...
#'
#' @param rep A fitted SPiCT report object (`spictcls`).
#' @param color,linetype,linewidth,lineend Styling for the vline(s).
...
#' - No `maninterval`: single line at the last observed catch time (if known).
#' - With `maninterval`: line at last observed catch start (or `left - dt`), plus line at left boundary.
#'
#' @param rep A fitted SPiCT report object (`spictcls`).
...
#'
#' @param rep_man A fitted SPiCT report object (`spictcls`), possibly with scenarios.
#' @param scenario_color Optional named vector of colours for scenarios; otherwise uses `man_cols()`.
#' @param show_CIs Logical; draw base-fit CI ribbon/lines. Default `TRUE`.
#' @param CI Confidence level in (0, 1). Default `0.95`.
#' @param show_legend Logical; show/hide the scenario legend. Default `TRUE`.
#' @return A `ggplot` object.
#' @export
my_plot_manage_bbmsy_panel_sim <- function(rep_man,
                                           scenario_color = NULL,
                                           show_CIs = TRUE,
                                           CI = 0.95,
                                           show_legend = TRUE) {
  ...
  pB <- ggplot2::ggplot() +
    ...
    # --------- EXACT truth overlay like plotspict.bbmsy(): B(t)/Bmsy(t) in orange ----------
  { if ("true" %in% names(rep_man$inp)) {
    tr <- rep_man$inp$true
    ggplot2::geom_line(
      data = data.frame(time = tr$time, y = tr$B / tr$Bmsy),
      ggplot2::aes(x = time, y = y),
      inherit.aes = FALSE, color = true_col(), linewidth = 0.8, show.legend = FALSE
    )
  } } +
    # ---------------------------------------------------------------------------------------
  ...
  # ---------- ELU2-style one-step-ahead for raw fit (no $man) ----------
  if (!length(scs_final)) {
    BB_all <- elu2::get.par("logBBmsy", rep_man, exp = TRUE, CI = CI)
