...
#' \itemize{
#'   \item a grey band for the uncertainty on \eqn{B_{MSY}} over time,
#'   \item a blue transparent ribbon for \eqn{B/B_{MSY}} mapped onto the biomass scale,
...
#' @param show_CIs Logical; draw ribbons & dotted CI edges. Default `TRUE`.
...
plot_elu2_panel_biomass <- function(model,
  ...
  if (isTRUE(show_CIs)) {
    ...
    if (nrow(df_BB_rib)) {
      p <- p + geom_ribbon(data = df_BB_rib, aes(x = time, ymin = ymin, ymax = ymax), fill = grDevices::rgb(0,0,1,0.10)) +
        geom_line(data = transform(df_BB_rib, y = ymin), aes(x = time, y = y), color = grDevices::rgb(0,0,1,0.20), linewidth = 0.6) +
        geom_line(data = transform(df_BB_rib, y = ymax), aes(x = time, y = y), color = grDevices::rgb(0,0,1,0.20), linewidth = 0.6)
    }
  }
