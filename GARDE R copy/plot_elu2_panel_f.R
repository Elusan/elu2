...
#' @param show_CIs Logical; draw ribbons & CI edges.
...
plot_elu2_panel_f <- function(model,
  ...
  # Build the (F/Fmsy)×Fmsy ribbon mapped to absolute F
  ...
  if (is.matrix(fmsy_mult)) FFabs <- FFrel * fmsy_mult else FFabs <- FFrel * as.numeric(fmsy_mult)
  ribFF <- .spict_make_ribbon_df(timef, FFabs[seq_len(indxmax), 1], FFabs[seq_len(indxmax), 3])

  p <- ggplot()

  # Fmsy band (lightgray) + Fmsy mean line (black)
  if (isTRUE(show_CIs)) {
    p <- p + geom_ribbon(data = Fmsy_band, aes(x = time, ymin = ymin, ymax = ymax),
                         fill = "lightgray")
  }
  p <- p + geom_line(data = Fmsy_line, aes(x = time, y = y),
                     color = "black", linewidth = 0.7)

  # (F/Fmsy)×Fmsy ribbon: fill rgb(0,0,1,0.10), solid boundary lines rgb(0,0,1,0.20)
  if (isTRUE(show_CIs) && nrow(ribFF)) {
    p <- p +
      geom_ribbon(data = ribFF, aes(x = time, ymin = ymin, ymax = ymax),
                  fill = grDevices::rgb(0,0,1,0.10)) +
      geom_line(data = transform(ribFF, y = ymin), aes(x = time, y = y),
                color = grDevices::rgb(0,0,1,0.20), linewidth = 0.6) +
      geom_line(data = transform(ribFF, y = ymax), aes(x = time, y = y),
                color = grDevices::rgb(0,0,1,0.20), linewidth = 0.6)
  }

  # Sub-annual Ft (semi-transparent blue), if present
  ...
  # Ft CI lines and mean (estimation window): dashed CIs, solid mean
  if (nrow(dfF_in)) {
    if (isTRUE(show_CIs)) {
      p <- p +
        geom_line(data = dfF_in, aes(x = time, y = lwr),
                  linetype = "dashed", linewidth = 0.5, color = line_color) +
        geom_line(data = dfF_in, aes(x = time, y = upr),
                  linetype = "dashed", linewidth = 0.5, color = line_color)
    }
    p <- p + geom_line(data = dfF_in, aes(x = time, y = est),
                       color = line_color, linewidth = 0.8)
  }

  # Ft CI lines and mean (prediction window): dashed CIs, dotted mean
  if (!manflag && nrow(dfF_pr)) {
    if (isTRUE(show_CIs)) {
      p <- p +
        geom_line(data = dfF_pr, aes(x = time, y = lwr),
                  linetype = "dashed", linewidth = 0.5, color = line_color) +
        geom_line(data = dfF_pr, aes(x = time, y = upr),
                  linetype = "dashed", linewidth = 0.5, color = line_color)
    }
    p <- p + geom_line(data = dfF_pr, aes(x = time, y = est),
                       linetype = "dotted", color = line_color, linewidth = 0.8)
  }

  # Observed F points (qf-scaled)
  ...
  p <- p +
    labs(title = "Absolute fishing mortality", x = "Year", y = expression(F[t])) +
