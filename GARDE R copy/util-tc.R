...
#'
#' Simulates biomass trajectories under multiple constant \eqn{F} multiples
#' (e.g., \eqn{0.75 F_{MSY}}, \eqn{1.0 F_{MSY}}, …) and draws the time needed to
#' hit a target band around \eqn{B_{MSY}} (default 0.95). Adds light guides at
#' the target band and \eqn{B/B_{MSY}=1}.
#'
...
#' @examples
#' \dontrun{
#' plotspict.tc_gg(all_models$S1$S1P.SDM)
#' }
plotspict.tc_gg <- function(rep, main = "Time to Bmsy", stamp = get.version(), CI = 0.95) {
  ...
  # (kept) original dynamic positioning variables (unused by the final override below)
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::theme_light(base_size = 11) +                              # (kept)
    ggplot2::theme(                                                     # (kept)
    ...
    # --- NEW: apply your compact theme, then override legend to be an "awesome" in-panel box (top-right) ---
    .spict_theme_minimal_compact22() +
    ggplot2::theme(
      # lock legend inside top-right corner
