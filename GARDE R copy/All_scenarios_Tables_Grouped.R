...
#' @param scenarios Character vector of scenario names (default auto-detected from names(model_list)).
...
All_scenarios_Tables_Grouped <- function(
  ...
  if (is.null(scenarios) || length(scenarios) == 0) {
    ...
    # Extract leading 'S' + digits (e.g., S1, S10) from each element name
  ...
  # -- Helper: Pivot long summary to wide --
  ...
  # -- Helper: left-align value and CI inside the cell --
  ...
  for (scen in scenarios) {
    ...
    # -- Assign group and linespace/hline logic --
    ...
    df_out$add_linespace <- plain_parameters %in% c(
      tail(param_groups[[1]], 1),
      tail(param_groups[[2]], 1)
    )
    df_out$add_hline <- plain_parameters %in% c(
      head(param_groups[[2]], 1),
      head(param_groups[[3]], 1)
    )
    df_out$add_hline_row <- plain_parameters %in% c(
      "$\\sigma_C$", "$\\mathrm{MSY}$"
    )
    ...
    # -- Apply linespace and hline based on flags --
